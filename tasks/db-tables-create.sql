...
CREATE TABLE calendar_dates (
  service_id VARCHAR(32), -- calendar.service_id
  date DATE,
  exception_type SMALLINT
);

CREATE TABLE calendar (
  service_id VARCHAR(32), -- ESTIMATE
  monday BOOLEAN,
  tuesday BOOLEAN,
  wednesday BOOLEAN,
  thursday BOOLEAN,
...
CREATE TABLE stop_times (
  trip_id VARCHAR(64), -- trips.trip_id
  arrival_time TIME,
  departure_time TIME,
  stop_id VARCHAR(8), -- ESTIMATE
...
CREATE TABLE stops (
  ...
  parent_station VARCHAR(8) -- stop_id?
);

CREATE TABLE transfers (
  from_stop_id VARCHAR(8), -- stops.stop_id
  to_stop_id VARCHAR(8), -- stops.stop_id
  transfer_type SMALLINT,
  min_transfer_time SMALLINT -- seconds?
);

CREATE TABLE trips (
  route_id VARCHAR(5), -- routes.route_id
  service_id VARCHAR(32), -- calendar.service_id
  trip_id VARCHAR(64),
  trip_headsign VARCHAR(64), -- ESTIMATE
  direction_id SMALLINT,
  block_id VARCHAR(8), -- UNSURE
  shape_id CHAR(8) -- shapes.shape_id
);
