class SVG:
    ...
    def grid(self, x_inc, y_inc):
        ...
        return '<g id="grid-marks">%s</g>' % ' '.join([self.line(line, 'st0') for line in lines])
    ...
    def line(self, line, classname):
        p1, p2 = line
        x1, y1 = p1
        x2, y2 = p2
        return '<line class="{classname}" x1="{x1}" y1="{y1}" x2="{x2}" y2="{y2}" />'.format(
            x1 = x1,
            x2 = x2,
            y1 = y1,
            y2 = y2,
...
def draw(route):
    pen = SVG((1720, 1080), (24 * 60 * 60, len(route.get_stations())))
    ...
    {body}""".format(
        ...
        body = pen.body([
            ...
            pen.grid(30 * 60, 1),
            '<g id="routes">%s</g>' % \
                ''.join([pen.normalized_line(line, 'st1') for line in trip_lines])
        ])
    )
